...
\begin{itemize}
  ...
  each reverse step is a smaller, more accurate update, giving a much sharper, more coherent
  image. (Both stages used the same step count in this comparison, so the effect combines
  the stage-1 and stage-2 sampling quality.)
\end{itemize}
...
the average of all of them, which is exactly why the one-step result looks washed out. The iterative
...
\texttt{i\_start}$=20$ the output stays close to the input. In short, \emph{small}
\texttt{i\_start} gives the strongest, most creative edit and \emph{large} \texttt{i\_start}
keeps you faithful to the original. This is clearest on the hand-drawn input, which only
turns into a realistic photo at the \emph{small}-\texttt{i\_start} settings and stays close
to the drawing at \texttt{i\_start}$=20$.
...
injects \emph{semantic} content, not just ``realism.'' At \emph{small} \texttt{i\_start} the
image is heavily noised, so the model has enough freedom to generate a full rocket scene; as
\texttt{i\_start} \emph{increases} more of the campanile structure is preserved and the
output stays constrained by the original tower. The prompt controls which kind of real image
the result is pulled toward, so the chosen words change what the edit becomes, while
\texttt{i\_start} controls how strongly the input image constrains it.
